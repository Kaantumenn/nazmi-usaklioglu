...
export function HomeFooter() {
  return (
    <footer className="relative z-30 w-full shrink-0 bg-black py-5 sm:py-6 md:py-8">
      <div className="mx-auto flex w-full max-w-4xl flex-col items-center gap-3 px-4 sm:gap-4 sm:px-6">
        <div className="flex max-w-full flex-wrap items-center justify-center gap-3 sm:gap-4 md:gap-5">
          {socialLinks.map(({ label, href, icon }) => (
            <a
              ...
              aria-label={label}
              className="flex h-11 min-h-11 w-11 min-w-11 touch-manipulation items-center justify-center rounded-full border border-white/35 bg-white/5 text-white transition-colors hover:border-white/55 hover:bg-white/10 sm:h-12 sm:w-12 md:h-11 md:w-11"
            >
              {icon}
            </a>
          ))}
